...
export class SingleLinkedList<T> implements List<T> {
  ...
  /**
   * Creates a singly-linked list filled with the elements generated by the
   * given iterable.
   *
   * This method is in `O(n)`.
   *
   * ```ts
   * const l = DoubleLinkedList.from([1, 2, 3])
   * for (const el of l) {
   *   console.log(el);
   * }
   * ```
  ...
  /**
   * Creates a new singly-linked list that contains no elements.
   *
   * This method is in `O(1)`.
   *
   * ```ts
   * const l = SingleLinkedList.empty();
  ...
  *[Symbol.iterator](): IterableIterator<T> {
  ...
  clone() {
    return SingleLinkedList.from<T>(this);
  }
